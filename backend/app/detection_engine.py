...
class DetectionEngine:
    ...
    def analyze(self, data: dict, progress_cb=None) -> dict:
        ...
        for key, rows in artifacts:
            ...
            elif "mplog" in key_lower or "defender_mplogs" in key_lower:
    ...
    def _detect_eventlogs(self, key: str, rows: list[dict]):
        ...
        for idx, event in enumerate(rows):
            ...
            # Defender's Operational log is collected by the collector but
            # was never specifically analyzed before this — every other
            # event log gets a dedicated branch (logon, service install,
            ...
            # available: it either caught something, or someone disabled it.
                ...
                # case where the configured action was "Allow" (detected
                # but NOT remediated).
                threat_match = re.search(r"(?:threat\s*name|Threat Name)[:\s]+([A-Za-z0-9:/_.\-]{3,100})", data_str, re.IGNORECASE)
    ...
    # ── Windows Defender MPLog (Microsoft Protection Log) ──

    def _detect_defender_mplogs(self, key: str, rows: list[dict]):
        """
        Process parsed MPLog events (see collector._parse_mplog_lines).

        Unlike the Defender Operational EVTX events (handled in
        _detect_eventlogs), MPLog events come with file paths, hashes, and
        PIDs already extracted, and include event types EVTX doesn't carry
        at all — EMS (memory-scan) detections and per-process file-access
        evidence (Estimated Impact) that can survive even after the
        offending process and file are both gone from disk.
        """
        for idx, event in enumerate(rows):
            ...
            if etype == "detection":
                ...
                # SDN events are file-existence + hash evidence from
                # Defender's cloud-lookup telemetry — present regardless of
                # whether a detection verdict was reached. Lower severity
                # than an actual detection, but valuable IOC material:
                # confirms a specific file (with hash) existed on the host.
                self._add_finding(
                    "suspicious_file", "low",
                ...
                # High file count on its own (regardless of path) — a
                # process touching tens of thousands of files in one scan
                # window is anomalous for almost anything except backup/AV/
                # indexing software, and worth surfacing for the analyst to
                # corroborate against what the process actually is.
