...
def correlate_behavior(engine) -> dict:
    ...
    # correlated_risk findings (see risk_scoring.py) are themselves a
    # strong signal — a process with multi-category corroboration earned
    # its own summary finding, which is worth surfacing explicitly here
    # rather than relying on the analyst to notice it in the findings list.
    risk_summaries = [f for f in engine.findings if f["category"] == "correlated_risk"]
    if risk_summaries:
        chains.insert(0, f"{len(risk_summaries)} process(es)/entit(ies) show multi-category "
                     f"corroborated risk — see 'Cumulative risk' findings for details.")
